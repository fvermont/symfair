...
class Dao
{
  ...
  function addClient($aParams) {
    ...
    //
    return $this->dataLink->query($sQuery, $aParams);
  }
  ...
  function getClients($aParams=array()) {
    ...
    //
    return $this->dataLink->getResultSet($sQuery);
  }
  //
  //------------------------------------------------- vols
  function getVolGs() {    
  ...
  function getVolRes($aParams) {    
    ...
    //
    $a = $this->dataLink->getResultSet($sQuery, $aParams);
  ...
  function getResLastNum($iAgence) {    
    ...
    //
    $a = $this->dataLink->getResultSet($sQuery);
  ...
  function addReservation($aParams) {
    ...
    if(empty($aParams[':gnc_id']))
      $aParams[':gnc_id'] = 1;
    //
    $aParams[':rsr_num'] = $this->getResLastNum($aParams[':gnc_id'])+1;
  ...
  function getReservation($aParams) {
    $sQuery = "SELECT r.gnc_id, code_agence, rsr_num,
      ...
      FROM reservation r join client c on r.cln_id = c.cln_id
      join agence a on a.gnc_id=r.gnc_id
      join vol v on v.date_dep = r.date_dep and v.vlg_num = r.vlg_num
      join vol_g g on v.vlg_num = g.vlg_num
      join aeroport ad on ad.code=g.code_arp_dep
      join aeroport aa on aa.code=g.code_arp_arr
      where r.gnc_id = :gnc_id and rsr_num = :rsr_num";
    //
    $a = $this->dataLink->getResultSet($sQuery, $aParams);
    if( is_array($a) && isset($a[0]) && is_array($a[0]))
      return $a[0];
  ...
  function getCredendials($login, $pwd) 
  {
    $sQuery = "select * from agence where code_agence = :login and mot_de_passe = :pwd";
    //
    $res = $this->dataLink->getResultSet($sQuery, 
      [':login' => $login, ':pwd' => $pwd]);
    //
